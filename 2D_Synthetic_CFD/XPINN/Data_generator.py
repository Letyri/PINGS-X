...
def load_data_burgers_Grid(Data_res=2,save_fig=True):
    ...
    sample_ratio = int(Data_res)
    
    ### Boundary
    xb = np.concatenate([x[::sample_ratio], np.zeros(len(y[::sample_ratio]))+1, np.zeros(len(y[::sample_ratio]))-1])
    yb = np.concatenate([np.zeros(len(x[::sample_ratio])), y[::sample_ratio], y[::sample_ratio]])
    ub = np.concatenate([u[::sample_ratio, ::sample_ratio][:, 0], u[::sample_ratio, ::sample_ratio][-1, :], u[::sample_ratio, ::sample_ratio][0, :]])
    
    y, x = np.meshgrid(y, x)
    x, y, u_s = x.reshape(-1), y.reshape(-1), u.reshape(-1)
    f = np.zeros(len(x))
        
    x_2d = x.reshape(len(data['x']), len(data['t']))
    y_2d = y.reshape(len(data['x']), len(data['t']))
    u_s_2d = u_s.reshape(len(data['x']), len(data['t']))
    f = f.reshape(len(data['x']), len(data['t']))
    
    # Sample uniformly
    xf = x_2d[::sample_ratio, ::sample_ratio].flatten()
    yf = y_2d[::sample_ratio, ::sample_ratio].flatten()
    uf = u_s_2d[::sample_ratio, ::sample_ratio].flatten()
    ff = f[::sample_ratio, ::sample_ratio].flatten()
    
    
    if save_fig:
        plt.rcParams["figure.figsize"] = (6.0,5.0)
        plt.tricontourf(xf,yf, uf.reshape(-1) ,100,cmap='viridis')
        plt.xlabel('x')
        plt.ylabel('t')
        ...
        plt.rcParams["figure.figsize"] = (6.0,5.0)
        plt.scatter(xf,yf, c=uf.reshape(-1),cmap='viridis')
        plt.xlabel('x')
        plt.ylabel('t')
        plt.title('Burgers u(x,t)')
        plt.colorbar()
...
def load_data_BB_Grid(Data_res,Num_data:int=201,x0=-10, x1=15, t0=-3, t1=2, save_fig=0):
    ...
    x_L = x1 - x0
    t_L = t1 - t0
    x, t = np.linspace(x0, x1, Num_data), np.linspace(t0, t1, Num_data); x, t = np.meshgrid(x, t); x, t = x.reshape(-1), t.reshape(-1);
    u, v = func_u(x, t), func_v(x, t)

    xb = np.concatenate([np.linspace(x0, x1, Num_data), np.linspace(x0, x1, Num_data), np.zeros(Num_data) + x0, np.zeros(Num_data) + x1], axis=0)
    tb = np.concatenate([np.zeros(Num_data) + t0, np.zeros(Num_data) + t1, np.linspace(t0, t1, Num_data), np.linspace(t0, t1, Num_data)], axis=0)
    ub, vb = func_u(xb, tb), func_v(xb, tb)
    ...
    x_2d = x.reshape(Num_data,Num_data)
    t_2d = t.reshape(Num_data,Num_data)
    u_s_2d = u.reshape(Num_data,Num_data)
    v_s_2d = v.reshape(Num_data,Num_data)
    
    xf = x_2d[::sample_ratio, ::sample_ratio].flatten()
    tf = t_2d[::sample_ratio, ::sample_ratio].flatten()
    uf = u_s_2d[::sample_ratio, ::sample_ratio].flatten()
    vf = v_s_2d[::sample_ratio, ::sample_ratio].flatten()
    ...
    if save_fig:
        ...
        plt.tricontourf(triang_coarse, u, 100 ,cmap='viridis', extend="both")
        plt.xlabel('x')
        plt.ylabel('t')
        plt.title('Burgers u(x,t)')
        plt.colorbar()
        ...
        plt.rcParams["figure.figsize"] = (6.0,5.0)
        triang_coarse = tri.Triangulation(x, t)
        plt.tricontourf(triang_coarse, v, 100 ,cmap='viridis', extend="both")
        plt.xlabel('x')
        plt.ylabel('t')
...
def load_data_BB(Num,x0=-10, x1=15, t0=-3, t1=2, save_fig=0):
    p = 1; q = 1; beta = 1; p1 = 2; q1 = p1*(2*q+p1*p+2*p**2)/(2*p)
    x, t = sy.symbols('x, t'); w = p * x + q * t + 0.5 * sy.log(1 + sy.exp(p1 * x + q1 * t))
    u1 = w.diff(x) / 2
    u0 = (2 * w.diff(t) - w.diff(x).diff(x)) / (4 * w.diff(x))
