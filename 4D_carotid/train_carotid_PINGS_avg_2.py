...
def main(args):
    ...
    for iters in tqdm(range(1, epochs + 1)):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            u_xx = torch.autograd.grad(u_x.sum(), batch_input, create_graph=True)[0][:, 0:1]
            u_yy = torch.autograd.grad(u_y.sum(), batch_input, create_graph=True)[0][:, 1:2]
            u_zz = torch.autograd.grad(u_z.sum(), batch_input, create_graph=True)[0][:, 2:3]

            v_xx = torch.autograd.grad(v_x.sum(), batch_input, create_graph=True)[0][:, 0:1]
            v_yy = torch.autograd.grad(v_y.sum(), batch_input, create_graph=True)[0][:, 1:2]
            v_zz = torch.autograd.grad(v_z.sum(), batch_input, create_graph=True)[0][:, 2:3]

            w_xx = torch.autograd.grad(w_x.sum(), batch_input, create_graph=True)[0][:, 0:1]
            w_yy = torch.autograd.grad(w_y.sum(), batch_input, create_graph=True)[0][:, 1:2]
            w_zz = torch.autograd.grad(w_z.sum(), batch_input, create_graph=True)[0][:, 2:3]
