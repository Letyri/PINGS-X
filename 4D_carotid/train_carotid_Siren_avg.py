...
from utils import Gaussian,dataprocess_MRI,utils,NS_Equation
from PINN import model_4D
import wandb 
import argparse

def get_args():
    parser = argparse.ArgumentParser(description='Train Siren model on carotid artery data.')
    parser.add_argument('--data_dir', type=str, default='./Data/carotid/MRV_2.mat', help='Path to the training data file.')
    parser.add_argument('--model_dir', type=str, default='./Results_carotid_Siren_avg_2', help='Directory to save model weights and results.')
    ...
    # parser.add_argument('--wandb_name', type=str, default='carotid_Siren_avg', help='Wandb run name.')
...
def main():
    ...
    Model_2 = model_4D.PINN(4,4,256,8,'Sin',20,False,10).to(device)
    ...
    utils.train_in_batches_PINN(Model_2,optimizer,x,y,z,t,data_num_1,train_gt, criterion, Re,L_gt,U_gt,epochs, 10000,Model_dir)
    end_time = time.time()
